<script>
  ...
  function addReply(inquiryIndex) {
    if (newReply.trim() !== "") {
      ...
      console.log("Current Replies: ", updatedProduct.inquiries[inquiryIndex].replies);
    } else {
      alert("대댓글 내용을 입력하세요.");
    }
  }
...
<main>
  ...
  <form on:submit|preventDefault={handleSubmit} class="mt-4">
    <!-- 상품 정보 입력 필드 -->
    <div>
      <label for="product-name" class="block text-sm font-medium">상품 이름</label>
      <input
        id="product-name"
        type="text"
    ...
    <div class="mt-2">
      <label for="product-seller" class="block text-sm font-medium">판매자</label>
      <input
        id="product-seller"
        type="text"
    ...
    <div class="mt-2">
      <label for="product-rating" class="block text-sm font-medium">판매자 평점</label>
      <input
        id="product-rating"
        type="number"
    ...
    <div class="mt-4">
      <button type="submit" class="bg-blue-500 text-white p-2 rounded">수정</button>
    </div>
  </form>
  ...
  {#if activeTab === 'reviews'}
    ...
    <ul>
      {#each updatedProduct.reviews as review}
        <li class="border-b py-2">
          ...
          <span class="text-gray-500">{review.date.toLocaleDateString()}</span> - {review.rating} ⭐
        </li>
      {/each}
    </ul>
    ...
    <button on:click={addReview} class="bg-green-500 text-white p-2 rounded mt-2">후기 추가</button>
  {/if}
  ...
  {#if activeTab === 'inquiries'}
    ...
    <ul>
      {#each updatedProduct.inquiries as inquiry, index}
        <li class="border-b py-2">
          ...
          <button on:click={() => currentInquiryIndex = index} class="text-blue-500 ml-2">답변하기</button>
          {#if currentInquiryIndex === index}
            <div class="mt-2">
              <input
                ...
                class="border rounded p-2 w-full"
              />
              <button on:click={() => addReply(index)} class="bg-green-500 text-white p-2 rounded mt-2">답변 추가</button>
            </div>
          {/if}
          <ul>
            {#each inquiry.replies as reply}
              <li class="ml-4 border-l pl-2 py-1">
                ...
                <span class="text-gray-500">{reply.date.toLocaleDateString()}</span>
              </li>
            {/each}
          </ul>
        </li>
    ...
    <button on:click={addInquiry} class="bg-green-500 text-white p-2 rounded mt-2">문의 추가</button>
  {/if}
</main>
